...
class WorkerRuntime:
    ...
    def select_large_format_batch_jobs(self, waiting_jobs: list[LargeFormatJob]) -> list[LargeFormatJob]:
        a_series_jobs = [job for job in waiting_jobs if classify_a_series_large_format_job(job) in {"a1", "a2", "a3"}]
        if a_series_jobs:
            return a_series_jobs
        return waiting_jobs
    ...
    def process_large_format_batches(self) -> None:
        ...
        if len(selected_jobs) != len(waiting_jobs) and all(classify_a_series_large_format_job(job) in {"a1", "a2", "a3"} for job in selected_jobs):
            message += " A-series jobs were batched using dedicated cut panels."
        if deferred_count:
