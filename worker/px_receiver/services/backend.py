...
class BackendClient(ABC):
    ...
    @abstractmethod
    def search_jobs(self, query: str) -> list[JobRecord]:
        raise NotImplementedError
...
class MockBackendClient(BackendClient):
    ...
    def search_jobs(self, query: str) -> list[JobRecord]:
        ...
        return [
            ...
            if job.order_id.strip().casefold() == normalized or job.id.strip().casefold() == normalized
        ]
...
class HttpBackendClient(BackendClient):
    ...
    def search_jobs(self, query: str) -> list[JobRecord]:
        encoded_query = parse.urlencode({"query": query})
        scoped_endpoint = f"/api/receiver/orders/search?{parse.urlencode({'query': query, 'machine_id': self.settings.machine_id})}"
        unscoped_endpoint = f"/api/receiver/orders/search?{encoded_query}"

        try:
            payload = self._request("GET", scoped_endpoint, headers=self._machine_headers)
        except Exception as exc:  # noqa: BLE001
            if "HTTP 404" not in str(exc):
                raise
            try:
                payload = self._request("GET", unscoped_endpoint, headers=self._machine_headers)
            except Exception as fallback_exc:  # noqa: BLE001
                if "HTTP 404" in str(fallback_exc):
                    return []
                raise

        results = payload if isinstance(payload, list) else payload.get("results", payload.get("jobs", payload))
        if not isinstance(results, list):
            return []

        return [_job_from_payload(item, self.settings.machine_id) for item in results if isinstance(item, dict)]
...
def _job_from_payload(item: dict[str, Any], machine_id: str) -> JobRecord:
    ...
    return JobRecord(
        ...
        shipping_label_path=item.get("shipping_label_path", item.get("shippingLabelPath")),
