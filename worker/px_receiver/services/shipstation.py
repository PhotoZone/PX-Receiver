...
def _get_base_url() -> str:
    return os.getenv("SHIPSTATION_API_BASE", "https://api.shipstation.com").rstrip("/")


def _get_headers() -> dict[str, str]:
    api_key = (os.getenv("SHIPSTATION_API_KEY") or "").strip()
    if not api_key:
...
def _request_json(method: str, url: str, payload: dict | None = None) -> dict:
    ...
    req = request.Request(url, data=body, headers=_get_headers(), method=method)
    try:
...
def _normalize_label_response(payload: dict) -> bytes:
    if payload.get("labelData"):
        return base64.b64decode(payload["labelData"])

    if payload.get("labelDownload"):
        return _download_binary(str(payload["labelDownload"]))
    ...
    if isinstance(label_download, dict):
        ...
        if href:
            return _download_binary(str(href))
...
def find_shipment_id(order_number: str) -> str:
    base_url = _get_base_url()
    for param_name in ("shipment_number", "shipmentNumber"):
        page = 1
        while True:
            query = parse.urlencode({param_name: order_number, "page": page, "pageSize": 200})
            payload = _request_json("GET", f"{base_url}/v2/shipments?{query}")
            ...
            for shipment in shipments:
                ...
                if shipment_id and str(shipment_number) == str(order_number):
...
def create_shipping_label_pdf(*, shipment_id: str | None = None, order_number: str | None = None) -> Path:
    target_shipment_id = shipment_id or (find_shipment_id(order_number) if order_number else None)
    if not target_shipment_id:
        raise RuntimeError("No shipment ID is available for this order.")

    payload = _request_json("POST", f"{_get_base_url()}/v2/labels/shipment/{target_shipment_id}", {})
    label_bytes = _normalize_label_response(payload)
